...
void* client_conn(void* args)
{
    char* flag = &static_cast<th_client_args*>(args)->flag;
    int client_socket = static_cast<th_client_args*>(args)->client_socket;
    struct sockaddr_un clientSockAddr = static_cast<th_client_args*>(args)->clientSockAddr;
...
int main()
{
    ...
    params.client_socket = socket(AF_UNIX, SOCK_STREAM, 0);
    fcntl(params.client_socket, F_SETFL, O_NONBLOCK);

    params.clientSockAddr.sun_family = AF_UNIX;
    strcpy(params.clientSockAddr.sun_path, "srsock.soc");
